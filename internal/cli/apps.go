...
func runDeploy(ctx context.Context, args []string, jsonOutput bool, stdout, stderr io.Writer) error {
	...
	if len(args) != 0 {
		return fmt.Errorf("usage: neb app deploy [--tenant <slug>] [--detach]")
	}
	root, err := os.Getwd()
	if err != nil {
		return err
	...
	bundle, err := os.CreateTemp("", "nebulous-bundle-*.zip")
	if err != nil {
	...
	if published.PreviousID != nil {
		rollback = "neb app deployment rollback " + *published.PreviousID + " --tenant " + tenantSlug
	}
...
func runOpen(ctx context.Context, args []string, jsonOutput bool, stdout io.Writer) error {
	...
	if len(args) != 0 {
		return fmt.Errorf("usage: neb app open [--tenant <slug>]")
	}
	root, err := os.Getwd()
	if err != nil {
		return err
...
func runDeployment(ctx context.Context, args []string, jsonOutput bool, stdout io.Writer) error {
	...
	if len(args) != 2 || args[0] != "rollback" {
		return fmt.Errorf("usage: neb app deployment rollback <deployment-id> [--tenant <slug>]")
	}
	state, err := loadState()
	if err != nil {
		return err
...
func runGenerate(args []string, jsonOutput bool, stdout io.Writer) error {
	...
	if len(args) != 0 {
		return fmt.Errorf("usage: neb app codegen [--output <file>]")
	}
	if output == "" {
		output = "nebulous.generated.ts"
	}
...
func generateTypeScript(m manifest.Manifest) string {
	...
	output.WriteString("// Generated by neb app codegen. Do not edit.\n\n")
	output.WriteString("export interface NebulousQueries {\n")
	...
	output.WriteString("}\n\nexport type NebulousQueryName = keyof NebulousQueries;\n")
