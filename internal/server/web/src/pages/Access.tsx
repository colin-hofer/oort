...
import {api, type ApiToken, type Member, type Tenant} from '../api';
...
export default function Access({tenant}: {tenant: Tenant}) {
  ...
  const load = useCallback(async () => {
    try {
      const [memberResult, tokenResult] = await Promise.all([api.listMembers(tenant.slug), api.listTokens(tenant.slug)]);
  ...
  const add = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await api.addMember(tenant.slug, String(values.get('email')), String(values.get('role')) as Member['role']);
      form.reset(); await load(); toast('Member added');
